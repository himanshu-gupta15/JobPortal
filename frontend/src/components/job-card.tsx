...
// import { ArrowRight, Briefcase, Building2, CheckCircle, DollarSign, Link, MapPin } from 'lucide-react'
...
//                     <h3 className='text-xl font-bold mb-2 line-clamp-2 group-hover:text-blue-600 transition-all'>
//                         {job.title}
...
//                         applied ? (<div className='flex-1 flex items-center justify-center gap-2 text-green-600 font-medium text-sm bg-green-100 dark:bg-green-900/30 rounded-md px-3 py-2'>
//                             <CheckCircle size={15}/>
//                             Applied 
...
//                     {job.is_active !==false &&( 
//                         <Button disabled={btnLoading} onClick={()=>applyJobHandler(job.job_id)} className='flex-1 gap-2'>
//                             <Briefcase size={16}/>
//                             Easy Apply 
//                             </Button>
//                 )}
...
import React, { useEffect, useState } from 'react';
import { Card, CardContent, CardHeader } from './ui/card';
import { ArrowRight, Briefcase, Building2, CheckCircle, DollarSign, Link as LinkIcon, MapPin } from 'lucide-react';
import { Button } from './ui/button';
...
const JobCard: React.FC<JobCardProps> = ({ job }) => {
    ...
    useEffect(() => {
        if (applications && job.job_id && Array.isArray(applications)) {
                ...
                <div className='flex w-full gap-2'>
                    ...
                    {user && user.role === "jobseeker" && (
                        <div className='flex-1 flex'>
                            {applied ? (
                                <div className='w-full flex items-center justify-center gap-2 text-green-600 font-medium text-sm bg-green-100 dark:bg-green-900/30 rounded-md px-3 py-2'>
                                    <CheckCircle size={15} />
                                    Applied
                                </div>
                            ) : (
                                job.is_active !== false && (
                                    <Button disabled={btnLoading} onClick={() => applyJobHandler(job.job_id)} className='w-full gap-2'>
                                        <Briefcase size={16} />
                                        Easy Apply
                                    </Button>
                                )
