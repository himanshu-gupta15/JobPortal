...
export const updateProfilePic = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  console.log("UPLOAD_SERVICE_URL:", process.env.UPLOAD_SERVICE);
  ...
  const [updatedUser] = await sql`
    ...
    WHERE user_id = ${user.user_id}
    RETURNING user_id, name, email, phone_number, role, bio, resume, resume_public_id, profile_pic, profile_pic_public_id, subscription;
  `;
...
export const addSkillToUser=TryCatch(async(req:AuthenticatedRequest, res)=>{
  ...
  if(!skillName || skillName.trim()===""){
    throw  new ErrorHandler(400,"Skill provide a skill name");
...
export const deleteSkillFromUser=TryCatch(
  async(req:AuthenticatedRequest,res)=>{
    ...
    res.json({
      message:`Skill ${skillName.trim()} is removed successfully`,  
    })
  }
)
...
export const applyForJob=TryCatch(async(req:AuthenticatedRequest,res)=>{
  ...
  if(!resume){
    throw new ErrorHandler(400,"You need to add resume in you profile to apply for this job")
  }

  const {job_id}=req.body;
  if(!job_id){
    throw new ErrorHandler(400,"Please provide job_id to apply for a job");
  }
  ...
  try{
    ...
    if(error.code==="23505"){
      throw new ErrorHandler(400,"you have already applied for to this job.");
  }
  throw error;
}
  res.json({
